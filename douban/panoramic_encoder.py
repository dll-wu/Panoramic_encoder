...
class PanoramicEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.lossfn2 = nn.BCEWithLogitsLoss()
    ...
    def forward(self,
        ...
        for i in range(Bs):
            ...
            idx = torch.arange(valid_length).unsqueeze(0).expand(len(bos_locations[i]), -1).to(device) # 8 * vL (assume num of res = 8)
            bos_pos = torch.LongTensor(bos_locations[i]).repeat(2, 1).to(device)
            bos_pos[1] = torch.roll(bos_pos[1], -1)
            bos_pos[-1, -1] = valid_length
            bos_pos = torch.transpose(bos_pos, 0, 1)  # 8 * 2
            idx = (idx >= bos_pos[:, 0].unsqueeze(1)) & (idx < bos_pos[:, 1].unsqueeze(1)) # 8 * vL
            ...
            ## history attn to itself,  responses attn to themselves
        ...
        ## You can deside if you would like to add one attention step.
        ...
        for i in range(len(tokens)):
            valid_length = (attention_mask[i] == 1).sum().item()
            idx = torch.arange(L).unsqueeze(0).expand(len(bos_locations[i]), -1).to(device)  # 8 * L
            bos_pos = torch.LongTensor(bos_locations[i]).repeat(2, 1).to(device)
            bos_pos[1] = torch.roll(bos_pos[1], -1)
            bos_pos[-1, -1] = valid_length
        ...
        attention_output = torch.stack(mean_reps, dim=0) # b*8*hs
        mean_logits = torch.squeeze(self.fc(attention_output)) # projection bs*8
        if labels != None : 
