...
def build_input_from_segments(persona, history, responses, tokenizer):
    bos, eos, yourself, partner = tokenizer.convert_tokens_to_ids(
        SPECIAL_TOKENS
    )
    sequence = history 
    sequence = [
        [yourself if i % 2 == 1 else partner] + s  # every history: start with partner, end with partner
        for i, s in enumerate(sequence)
    ]
    sequence_p = [[bos]] + persona + [[eos]]  
    sequence_p = list(chain(*sequence_p))
    sequence = sequence  + [[eos]]
    # we should cut the sequence: less than 512
    # max_response_length = max([len(res)+2 for res in responses])
    max_response_length = max([len(res)+2 for res in responses])
    max_response_length = min(max_response_length, 256)
    while True:
        sequence_his = list(chain(*sequence))
        if len(sequence_his) + len(sequence_p) <= 256:
            break
        sequence = sequence[1:]
    instance = {}
    instance["input_ids"] = sequence_p + sequence_his
...
def rank_responses(persona, history, responses, tokenizer, model, args):
    special_tokens_ids = tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS)    
    instance = build_input_from_segments(
        persona, history, responses, tokenizer
    )
...
def rank_label(persona, records, replies, tokenizer, model, args):
    ...
    with torch.no_grad():
        pred_label, descend_labels = rank_responses(persona, history, responses, tokenizer, model, args)
    return pred_label, descend_labels
...
class Ranker:
    ...
    def rank(self, persona, records, responses):
        return rank_label(persona, records, responses, self.tokenizer, self.model, self.args)
